...
import { openPrescriptionPdf } from "../../../../lib/prescriptions";
...
export function PrescriptionsPage() {
  ...
  const requestRefill = usePatientStore((state) => state.requestRefill);
  ...
  const latestOrderForMed = (medication: string) => {
    const q = medication.toLowerCase().split(" ")[0];
    return orders.find(
      (o) =>
        o.medication?.toLowerCase().includes(q) &&
        ["rx_sent", "shipped", "delivered", "refill_eligible"].includes(o.status),
    );
  };

  const handleRefill = async (rx: PrescriptionRecord) => {
    ...
    if (order.status === "refill_eligible") {
      toast.info("Your refill is already queued for physician review.");
      return;
    }
    setRefillBusyId(rx.id);
    const result = await requestRefill(order.id);
    setRefillBusyId(null);
    if (result.ok) {
      toast.success("Refill request submitted. Your physician will review it shortly.");
      await fetchOrders();
    } else {
      toast.error(result.error || "Could not submit refill request.");
    }
  };
  ...
  const getStatusConfig = (s: string) => {
    const config: Record<string, { label: string; color: string; bg: string }> = {
      ...
      discontinued: { label: "Discontinued", color: "text-slate-500", bg: "bg-slate-50" },
    };
      ...
      <div className="flex flex-col md:flex-row items-end justify-between gap-6 border-b border-slate-100 pb-10">
        <div className="space-y-4 text-center md:text-left">
          <div className="flex items-center justify-center md:justify-start gap-2">
            <Badge
              variant="outline"
              className="text-[8px] font-black uppercase tracking-widest border-emerald-100 bg-emerald-50 text-[#0A2E1F] px-3 py-1"
            >
              Clinical Ledger
            </Badge>
            <Badge
              variant="outline"
              className="text-[8px] font-black uppercase tracking-widest border-slate-100 bg-slate-50 text-slate-400 px-3 py-1"
            >
              PHI Encrypted
            </Badge>
          </div>
        ...
        >
          {(["active", "history", "all"] as const).map((f) => (
            ...
            >
              {f}{" "}
              {f === "active" && activePrescriptions.length > 0 && `(${activePrescriptions.length})`}
            </button>
          ))}
        </div>
      </div>
            ...
            {list.length === 0 ? (
              ...
              <div className="grid gap-4">
                {list.map((rx, idx) => {
                  ...
                  const doctorLabel = rx.doctor_name || "Your care team";
                  const busy = refillBusyId === rx.id;
                  return (
                    <motion.div
                      key={rx.id}
                          ...
                          <div className="flex flex-col lg:flex-row items-center gap-10">
                            <div
                              className={cn(
                                ...
                                config.bg,
                                "border-slate-50 group-hover:scale-105 transition-transform duration-500",
                              )}
                            >
                            ...
                            <div className="flex-1 min-w-0 space-y-4 text-center lg:text-left">
                              <div className="flex flex-col lg:flex-row items-center gap-4">
                                <h2 className="text-xl font-black text-[#0A2E1F] tracking-tight uppercase italic">
                                  {rx.medication}
                                </h2>
                                <Badge
                                  className={cn(
                                    "px-3 py-0.5 rounded-lg text-[8px] font-black uppercase border-none",
                                    config.bg,
                                    config.color,
                                  )}
                                >
                              ...
                              <div className="flex flex-wrap items-center justify-center lg:justify-start gap-6 text-[9px] font-bold text-slate-400 uppercase tracking-widest">
                                <span className="flex items-center gap-1.5">
                                  <Stethoscope size={12} className="text-emerald-500" /> {doctorLabel}
                                </span>
                                <span className="flex items-center gap-1.5">
                                  <MapPin size={12} className="text-blue-400" />{" "}
                                  {rx.pharmacy_name || "Partner pharmacy"}
                                </span>
                                <span className="flex items-center gap-1.5">
                              ...
                              <div className="p-4 bg-slate-50/50 rounded-xl border border-slate-50">
                                <p className="text-[10px] font-medium text-slate-600 italic">
                                  &ldquo;{rx.dosage}
                                  {rx.frequency ? ` · ${rx.frequency}` : ""}&rdquo;
                                </p>
                              </div>
                            </div>
                            ...
                            <div className="shrink-0 flex items-center gap-3">
                              <Button
                                ...
                                onClick={() => openPrescriptionPdf(rx, doctorLabel)}
                              >
                              ...
                              <Button
                                size="sm"
                                type="button"
                                disabled={rx.status !== "active" || busy}
                                className="h-10 rounded-xl bg-[#0A2E1F] hover:bg-emerald-950 text-white font-black uppercase text-[9px] tracking-widest px-6 shadow-lg shadow-emerald-900/10 gap-2 disabled:opacity-50"
      ...
      <div className="flex flex-col md:flex-row items-center justify-between gap-6 p-10 rounded-[2.5rem] bg-[#0A2E1F] text-white overflow-hidden relative group">
        ...
        <div className="relative z-10 flex items-center gap-6">
          ...
          <div>
            ...
            <p className="text-[10px] font-medium text-emerald-100/40 uppercase tracking-widest mt-1">
              24/7 Pharmacy & Medical Board Support
            </p>
          </div>
        </div>
