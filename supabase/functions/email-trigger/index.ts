 ...
 * Triggered by Supabase Database Webhooks on `orders` UPDATE (configure in Dashboard).
 */
  ...
  try {
    ...
    if (doctorId) {
      const { data: profile } = await supabase
        ...
        .eq("id", doctorId)
        .maybeSingle();
      if (profile?.calendly_url) bookingLink = profile.calendly_url;
    }

    const doctorName = (record.doctor as string) || "your assigned physician";

    // ── Video requested ──
    if (record.zoom_status === "requested" && oldRecord.zoom_status !== "requested") {
      const subject = `Action Required: Video consultation — ${doctorName}`;
      const html = `
      ...
      if (phone) {
        await sendTwilioSms(
          phone,
          `Peak Health: ${doctorName} requested a video visit. Book: ${bookingLink}`.slice(0, 300)
        );
      }
