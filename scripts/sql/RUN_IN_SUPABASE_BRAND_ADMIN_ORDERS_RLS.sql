-- Fix brand_admin orders visibility when legacy rows use sub_brand = 'Peak Health'
-- but JWT app_metadata.brand_id is the Peak platform UUID.
-- Run in Supabase SQL Editor, then re-login as brand admin.
...
SELECT 'brand_admin_orders_rls' AS check_name, public.sub_brand_matches_auth_brand('Peak Health') AS legacy_peak_match;
